...
def get_income_statement(ticker, limit, key, period):
    ...
    try:
        r = requests.get('{}{}?period={}?limit={}&apikey={}'.format(URL,ticker,period,limit,key))
...
def get_daily_prices(ticker, timeseries, key):
    ...
    try:
        r = requests.get('{}{}?timeseries={}&apikey={}'.format(URL,ticker,timeseries,key))
...
def get_enterprise_value(ticker, rate, key, period):
    ...
    try:
        r = requests.get('{}{}?period={}&limit={}&apikey={}'.format(URL,ticker,period,rate,key))
        return pd.DataFrame.from_dict(r.json())
    except requests.exceptions.HTTPError as e:
        print('Requesting Enterprise Value ERROR: ', str(e))
if __name__ == "__main__":
    key = 'e7af99614801827ed43a9285c8eff7f2'
    limit = 6
    period = 'annual'
    ticker = 'AAPL'
    IS = get_income_statement(ticker, limit, key, period)
    
    writer = pd.ExcelWriter('{}.xlsx'.format(ticker), engine='xlsxwriter')
